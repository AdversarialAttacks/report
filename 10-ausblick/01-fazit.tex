...
In diesem Kapitel werden die Auswirkungen der Ergebnisse der Bachelorthesis, wichtige Aspekte, mögliche fortführende Forschungen und weitere Verteidigungsmechanismen diskutiert. 

\subsection{Auswirkungen unserer Ergebnisse}

Die Ergebnisse dieser Thesis unterstreichen die anhaltende Herausforderung bei der \Gls{robustifizierung} von Modellen gegen \acrlong{uap}s (\acrshort{uap}s) und Adversarial Attacks im Allgemeinen. Der entwickelte Robustifizierungsmechanismus kann nur bedingt ein Modell gegen Angriffe schützen, wodurch die Notwendigkeit besteht, weiterhin intensive Forschung in diesem Bereich zu betreiben.
...
Wie im Kapitel \ref{chap:Universal Adversarial Attacks auf Bildklassifikation} beschrieben, ist der in dieser Thesis beschriebener Perturbationsalgorithmus eine White-Box-Attacke. Im Paper von Moosavi-Dezfooli et al. \cite{moosavi-dezfooli_universal_2017} wird jedoch auch die Generalisierbarkeit thematisiert. Sie stellten fest, dass \acrshort{uap}s, die für ein bestimmtes Modell $A$ (z.B. VGG-19) generiert wurden, auch bei einem anderen Modell $B$ (z.B. ResNet-152) zu Fehlklassifikationen führen können.
...
In der Thesis konnten beispielsweise Ähnlichkeiten zwischen den \acrshort{uap}s von DenseNet- und ResNet-Modellen beim COVIDx CXR-4 Datensatz erkannt werden. Dies könnte auf eine \acrshort{uap} hindeuten, welche beide Modellarchitekturen auf diesem Datensatz täuschen könnte.
...
Bei den generierten \acrlong{uap}s für das Modell ResNet50 auf den Datensatz COVIDx CXR-4, in der Abbildung \ref{fig:bias-detektion}, ist zu erkennen, dass die Perturbationen hauptsächlich in der oberen rechten Ecke auftreten.
...
\begin{figure}[H]
    ...
    \end{subfigure}\hfill%
    \caption{Darstellung generierter UAPs für das auf den COVIDx CXR-4 Datensatz trainierte ResNet50 Modell.}
    \label{fig:bias-detektion}
\end{figure}

Da der Perturbationsalgorithmus hauptsächlich an dieser Stelle angreift, könnte daraus geschlossen werden, dass dieser Bildbereich für die Klassifikation sehr wichtig ist. Sollte dies tatsächlich der Fall sein, könnte die Erzeugung solcher \acrshort{uap}s alternativ zu Grad-CAM zur Aufdeckung von Bias oder zur Erklärbarkeit von Bildklassifikationsmodellen verwendet werden. Die hier getroffenen Annahmen müssten jedoch zuerst überprüft werden.
...
Während bei binären Klassifikationen hauptsächlich die Binary Cross Entropy als Loss-Funktion dient, wird bei Multiklassifikationsmodellen meistens die Cross Entropy Funktion verwendet. Die Cross Entropy Funktion eignet sich für die Bewertung von Wahrscheinlichkeitsverteilungen über alle Klassenkategorien hinweg.

\newpage

Für die Täuschung von Multiklassifikationsmodellen schlagen wir folgende Formel vor:

\begin{equation}
    L_{\text{UAP,U}} = \frac{1}{L_{\text{CE,UAP}}(y_i, \hat{y}_{i,\text{adv}}) + \epsilon} + \lambda_{norm} \cdot ||v + \Delta v||_p 
...
\begin{equation}
    L_{\text{UAP,T}} = L_{\text{CE,UAP,T}}(y_{i,\text{wrong}}, \hat{y}_{i,\text{adv}}) + \lambda_{norm} \cdot ||v + \Delta v||_p 
...
Wobei $y_{i,\text{wrong}}$ für ein One-Hot-Encoded Label steht, welches einen falschen Klassenwert darstellt ($y_{i,\text{wrong}} \neq y_{i}$).

\subsection{Weitere Verteidigungsmechanismen} \label{chap:Weitere Verteidigungsmechanismen}
In dieser Thesis wird im Kapitel \ref{chap:adversarial training} beschriebene Adversarial Training verwendet, um Klassifikationsmodelle robuster gegen Adversarial Attacks zu machen. In den letzten Jahren wurde an weiteren Methoden geforscht, um Modelle gegen Adversarial Attacks zu schützen, welche auch gegen \acrlong{uap}s getestet werden können.

\subsubsection{Cropshift Adversarial Training} \label{chap:Data Augementation}
Lin Li und Michael W. Spratling \cite{li_data_2023} präsentieren 2023 eine Data Augmentation Adversarial Training Methode namens Cropshift die im Gegensatz zu allen bisherigen Data Augmentation Adversarial Training Methoden sowohl die Robustheit als auch die Genauigkeit des Modells verbessert. Cropshift schneidet zufällig Teile eines Bildes aus und verschiebt sie an verschiedene Positionen. Dies hilft, robustes Overfitting im Adversarial Training zu reduzieren.

\subsubsection{Ensembles mit Diversity Training} 
Sanjay Kariyappa und Moinuddin K. Qureshi \cite{kariyappa_improving_2019} zeigen, dass die Robustheit von \acrlong{dnn}s gegenüber Adversarial Attacks verbessert werden können, indem Ensembles von Modellen mit unkorrelierten Verlustgradienten trainiert werden. Dies wird durch Diversity Training erreicht, welche das gemeinsame Adversarial Subspace der Modelle reduziert, die Transferierbarkeit der Attacken verringert und somit das Ensemble gegenüber Adversarial Attacks robuster ist. 
...
Paula Harder et al. haben in ihrer Arbeit SpectralDefense: Detecting Adversarial Attacks on CNNs in the Fourier Domain, \cite{harder_spectraldefense_2021} eine Methode zur Detektion von Adversarial Attacks vorgestellt. Die Detektion dient als Verteidigungsstrategie, um potenziellen Attacken zu verhindern. In ihrer Untersuchung werden die Fourier-Transformation von Bild- und Feature-Map-Daten genutzt, um zwischen normalen und manipulierten Bildern zu unterscheiden, wobei zwei Ansätze verfolgt werden. Der erste Ansatz fokussiert sich auf die Analyse des Magnituden-Spektrums der Eingangsbilder, während der zweite Ansatz zusätzlich die Phase der Fourier-Koeffizienten in verschiedenen Netzwerkschichten berücksichtigt.

\subsubsection{Bildmaske}
In den beiden verwendeten Datensätzen sind für die Klassifikation unwichtige schwarze Pixel im Hintergrund vorhanden, die potenziell eine Angriffsfläche für Adversarial Attacks darstellen. Um dieses Problem zu adressieren, schlagen wir die Verwendung einer Bildmaske vor. Diese würde dem Modell ermöglichen, sich ausschliesslich auf die wesentlichen Bereiche des Bildes zu konzentrieren und die für die Klassifikationsaufgabe irrelevanten Informationen zu ignorieren. Die Verwendung einer solchen Maske könnte dazu führen, dass das Modell Perturbationen ausblendet und somit die Angriffsfläche verringert.
...
Daher ist es von entscheidender Bedeutung darauf hinzuweisen, dass Experten, die ähnliche Krankheitserkennungssysteme verwenden, sich auch in Zukunft der Tatsache bewusst sein werden, dass solche Systeme auch bei Vorhandensein oder Nichtvorhandensein von \acrlong{uap}s nicht vollständig exakt sind und dass sie als Experten dieser Leichtgläubigkeit zum Opfer fallen könnten. Diese Experten sollten unbedingt \gls{in-the-loop} bleiben und den Output dieser Modelle sowie die eigene Nutzung dieser Systeme stets kritisch hinterfragen.